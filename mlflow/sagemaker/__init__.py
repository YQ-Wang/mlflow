...
def deploy_transform_job(
    ...
    :param assemble_with: The method to assemble the results of the transform job as a single S3
                            object.
...
def terminate_transform_job(
    ...
    :param archive: If ``True``, resources associated with the specified batch transform job,
                    ...
                    ``terminate_transform_job()`` must be executed synchronously with
                    ``synchronous=True``.
    ...
    if (not archive) and (not synchronous):
        raise MlflowException(
            message=(
                "Resources must be archived when `terminate_transform_job()` is executed in \
...
def _create_sagemaker_transform_job(
    ...
    :param model_uri: URI of the MLflow model to associate with the specified SageMaker
                    batch transform job.
    ...
    :param s3_output_path: The S3 path to store the output results of the Sagemaker transform job.
    :param accept: The multipurpose internet mail extension (MIME) type of the output data.
    :param assemble_with: The method to assemble the results of the transform job
                         as a single S3 object.
    :param input_filter: A JSONPath expression used to select a portion of the input data
                        for the transform job.
    :param output_filter: A JSONPath expression used to select a portion of the output data
